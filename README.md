...
5. 取りこぼしがあれば、耳で確認しながら手動で区間を追記(下記)
...
- **手動での区間追記**: 自動検出が抜けた箇所を人手で補える
  1. 動画を再生・シークして該当箇所の頭で「始点 [I]」(または `I` キー)
  2. 鳴き終わりで「終点 [O]」(または `O` キー)。始点・終点は
     スペクトログラム/スコアタイムライン上に黄色の線で表示される
  3. 「区間を追加 [A]」(または `A` キー)で結果テーブルに時系列順で挿入
  - 手動追加した行は黄色の背景で表示され、「種別」列が「手動」になる。
    解析済みならその区間のモデルスコアも併記される(未解析なら「-」)
  - 手動行を選択して「選択行を削除」で取り消せる(自動検出行は削除不可)
  - 再解析しても手動行は保持される。別の動画を読み込むとクリアされる
- **CSV 読込**: 保存した CSV を読み込み、手動追加した区間(`source=manual`)を
  現在の結果に取り込む(同じ開始・終了時刻の行はスキップ)。解析結果が
  まだ無いときは自動検出の行も読み込むので、過去の結果の閲覧にも使える
...
```csv
label,start_seconds,end_seconds,duration_seconds,start_time,end_time,max_score,mean_score,source
犬の鳴き声,3.360,5.280,1.920,0:00:03.360,0:00:05.280,0.912,0.774,auto
犬の鳴き声,12.150,13.400,1.250,0:00:12.150,0:00:13.400,,,manual
```
...
- `max_score` / `mean_score` … 区間内フレームスコアの最大値・平均値(0〜1)。
  手動追加行でスコアが無い場合は空欄
- `source` … `auto`(自動検出)/ `manual`(GUI で手動追加)。
  この列が無い旧形式の CSV も読み込め、すべて `auto` として扱われる
